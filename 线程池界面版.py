...
def process_images_recursively(directory):
    # 创建一个线程池，可以同时运行最多6个线程
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
        # os.walk函数会遍历directory目录及其所有子目录
        for root, dirs, files in os.walk(directory):
            # 对每个文件进行检查
            for filename in files:
                # os.path.splitext函数会将文件名分割成名字和扩展名
                ext = os.path.splitext(filename)[1].lower()# 我们取扩展名（index 1），并转为小写
                # 如果文件是图像文件（根据扩展名判断），则处理它
                if ext in ('.webp', '.jpg', '.jpeg', '.bmp', '.gif', '.png', '.PNG', '.WEBP', '.JPG', '.JPEG', '.BMP', '.GIF'):

                    image_path = os.path.join(root, filename)# 构建文件的完整路径
                    # 提交任务到线程池
                    # 这会使得forge_exif_data函数在一个新线程中被调用
                    executor.submit(forge_exif_data, image_path)          
