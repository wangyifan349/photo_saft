...
###################################
            ...
            ##################################
            create_time = os.path.getctime(image_path)# 获取文件创建时间            
            create_time = datetime.fromtimestamp(create_time)# 将创建时间转换为 datetime 对象
            random_days = random.randint(1, 30)# 生成随机天数（1到30之间）
            new_time = create_time - timedelta(days=random_days)
            ################################这里面可能没用可以删除
            # 随机选择一个方向
            direction = random.choice(['True North', 'Magnetic North', 'East', 'West'])
            # 创建 GPS 信息字典
            gps_ifd = {
        ...
        piexif.ImageIFD.Software: "IOS 15",

        # 以下是使用十六进制标签设置的特殊属性
        0x9c9b: b"wanglin@apple.com",  # XPComment
        0x9c9c: b"apple",  # XPCopyright
        piexif.ImageIFD.Software: u"13.1.2",
        piexif.ExifIFD.BodySerialNumber: u"123456789",  # 设置相机序列号，使用Unicode字符串
        piexif.ExifIFD.BodySerialNumber: b'cafw36caerMsasl5n9431',
        
        #用于记录图像处理软件的名称和版本信息，如果图像是由 Apple 设备生成的，则通常会将此字段设置为 "iOS xxx" 或 "macOS xxx"，其中 "xxx" 是操作系统的版本号。
        #例如，如果图像是由运行 iOS 15 的 iPhone 13 Pro Max 拍摄的，则 piexif.ImageIFD.Software 可以设置为 "iOS 15"。
        #piexif.ImageIFD.XPCopyright: b"Copyright 2023"  # 版权信息，同样需要是字节字符串
        }
            exif_info= {
        ...
        piexif.ExifIFD.DateTimeOriginal: timestamp.strftime("%Y:%m:%d %H:%M:%S"),  # 原始拍摄时间
        piexif.ExifIFD.DateTimeDigitized: timestamp.strftime("%Y:%m:%d %H:%M:%S"),  # 数字化时间
        piexif.ExifIFD.SceneCaptureType: 0,  # 标准场景拍摄类型
...
def is_directory_exist(path):
    if platform.system() == 'Windows':
        path = path.replace('/', '\\')  # 将斜杠替换为反斜杠
        path = path.replace('//', '\\\\')  # 将双斜杠替换为双反斜杠
    else:
        path = path.replace('\\', '/')  # 将反斜杠替换为斜杠
        path = path.replace('\\\\', '//')  # 将双反斜杠替换为双斜杠
...
def process_images_recursively(directory):
    ...
    for root, _, files in os.walk(directory):
        for filename in files:
            ...
            if ext in ('.webp', '.jpg', '.jpeg', '.bmp', '.gif', '.png'):
                all_images.append(os.path.join(root, filename))
    
    # 将所有图片分割为6个子列表
    image_lists = [[] for _ in range(6)]  # 创建包含6个空列表的列表，用于存储图像
    for i, image in enumerate(all_images):
        list_index = i % 6  # 计算当前图像应该分配到的列表索引
        # 如果当前图像应该分配到的列表索引大于剩余图像的数量，则将其分配到索引小于剩余图像数量的列表中
        if list_index >= len(all_images) % 6:
            list_index -= 1
        image_lists[list_index].append(image)  # 将图像附加到适当的列表中
    threads = []# 创建6个线程，每个线程处理一个子列表
    for image_list in image_lists:
        thread = threading.Thread(target=process_image_list, args=(image_list,))
        thread.start()  # 开始运行线程
...
user_name = os.getlogin()# 获取当前用户的用户名
image_directory = os.path.join("C:\\", "Users", user_name, "Pictures")# 获取图片目录的路径
print(image_directory)
image_directory = image_directory  # 在这里添加初始值
directory_label = Label(root, text="请选择图片文件夹：", wraplength=350)# 添加目录选择和显示组件
directory_label.pack(pady=10)
browse_button = Button(root, text="浏览", command=browse_directory)
browse_button.pack()
directory_entry = Entry(root, width=50)
directory_entry.pack(pady=5)
process_button = Button(root, text="开始处理", command=process_images_thread)# 添加开始处理按钮
process_button.pack(pady=10)
status_label = Label(root, text="")# 添加状态标签
status_label.pack(pady=5)
root.mainloop()# 运行GUI主循环
