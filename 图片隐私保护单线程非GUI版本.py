...
def process_images_recursively(directory):
    for root, ignored, files in os.walk(directory):
...
def process_images(directory):
    if is_directory_exist(directory):
        process_images_recursively(directory)
        print("处理完成: 已经清除了图片中的隐私信息，同时写入了一定的伪造信息")
    else:
        print("路径无法识别，有任何问题，请联系开发者。")
if __name__ == "__main__":
    directory = input("请输入图片文件夹的路径: ")
